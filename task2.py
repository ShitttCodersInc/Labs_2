...
Y = [3.3728, 2.4136, 1.2693, 0.3779, -0.0775, -1.8075, -0.8770, -2.6304, -5.0338, -5.5773,
...
S_b0 = lambda S_y_star, n, x_agv, S_x: S_y * (1/n + x_avg ** 2 / ((n-1) * S_x ** 2)) ** 0.5
S_b1 = lambda S_y_star, n, X: ((( S_y_star**2) * n) /(n * (sum(X[k] ** 2 for k in range(n))) - sum(X[k] for k in range(n)) ** 2 )) ** 0.5

delta_b0 = stats.f.ppf(alpha1, 48) * S_b0
delta_b1 = stats.f.ppf(alpha2, 48) * S_b1

print('Коэффициент b0 значим') if abs(b0) > abs(delta_b0) else print('Коэффициент b0 не значим')
print('Коэффициент b1 значим') if abs(b1) > abs(delta_b1) else print('Коэффициент b1 не значим')
...
F = S_y_out / S_y_ost
print('Уравнение регрессии адекватно для alpha = 0.01') if F > stats.t.ppf(alpha1, n-1, n-2) else print('Уравнение регрессии адекватно для alpha = 0.01')
print('Уравнение регрессии адекватно для alpha = 0.05') if F > stats.t.ppf(alpha2, n-1, n-2) else print('Уравнение регрессии адекватно для alpha = 0.05')
